...
- **Serpe ricorrente (trasversale a tutti gli archi)** — `artiglio.md` (il falco, il **serio**) · `cecca.md` + `sorcio.md` (il **braccio comico**: la gazza-spia tradita dal proprio occhio + il ratto che si crede un fuoriclasse) · `bissa.md` (l'erede, linea di successione) · `aspide.md` (il consigliere, il **ragno della rete**) · `il-mastino.md` (il vassallo fiero dei valichi, *piegabile per onore*). Sono i volti della Serpe che i protagonisti incontrano **più del vertice**: caratterizzati a fondo perché **non** siano cattivetti stereotipati. Costellazione: `../../cartografia/regni/pianura_alta/la_serpe.md`.
...
- `il-cittadino-del-leone.md` · `il-custode-delle-pietre-del-leone.md` · `la-lupa.md` · `la-pantera.md` · `il-marinaio-del-mare.md` — il **tessuto locale dell'Arco 6** (le Terre del Leone, **il FINALE**, città rivali): la tentazione della **gloria** (terzina dantesca) contro la **consegna** silenziosa, il **custode delle Pietre** che mostra il ritornello della pietra tiepida (+ **Toraki** aspetta qui), la **Lupa** di Siena (Cecco: aiuto sghembo, solidarietà fra reietti), la **Pantera** di Lucca (*di seta*, wildcard — + redde rationem del Cacciatore e ultima mano della Serpe), e il **marinaio** di Pisa («misura due volte», il barcaiolo delle partenze). Il finale: **annodare, posare, lasciare** — guardrail: *mostra la porta, non chi la attraversa*.
...
- ✅ **«i Disputanti» = classe ufficiale** in `_voci.json`/`voci_build.py` (`pianura_bassa`); `il-giovane-disputante.md` ripuntato.
- ✅ **Voci della Serpe complete**: aggiunte **Aspide** (`aspide.md`) e **il Mastino** (`il-mastino.md`).
- ☐ **Nomi dal lessico**: assegnare i nomi propri ai `<dal lessico>` di tutte le schede, in **un'unica passata dedicata**, dopo che la trama/`saga_graph` è definita (così i nomi restano coerenti). Vedi `../../lessico/MAPPATURA.md`.
